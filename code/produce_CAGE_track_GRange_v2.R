...
produce_disjoin_GRange_fn<-function(tmp_tbl){
  cage_Grange<-GRanges(seqnames=tmp_tbl$chrom,
                       ranges = IRanges(start=tmp_tbl$start,
                                        end=tmp_tbl$end
                       ))
  mcols(cage_Grange)<-tibble(score=tmp_tbl$score)
  
  rev_score<-findOverlaps(disjoin(cage_Grange),cage_Grange) %>% 
    as_tibble() %>% 
    mutate(score=mcols(cage_Grange)$score[subjectHits]) %>% 
    group_by(queryHits) %>% 
    summarise(m=mean(score))
  dis_cage_GRange<-disjoin(cage_Grange)
  mcols(dis_cage_GRange)<-tibble(score=rev_score$m)
  return(dis_cage_GRange)
}
...
########################################################################################################
out_tss_file<-"./data/H1_tss_peak_track_tbl.Rda"
out_enh_file<-"./data/H1_enh_peak_track_tbl.Rda"
...
out_enh_GRange<-produce_disjoin_GRange_fn(tmp_enh_tbl)
out_tss_GRange<-produce_disjoin_GRange_fn(tmp_tss_tbl)

save(out_enh_GRange,file=out_enh_file)
save(out_tss_GRange,file=out_tss_file)
